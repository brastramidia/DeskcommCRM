...
| `tarefas-pessoais.architecture.json` | área de Atividades › Tarefas — 17 peças, 35 arestas, 5 faixas; a porta na navegação, o badge de vencidas que é a única aresta de saída, e **três não-ligações declaradas** (a tarefa não aponta para lead/contato/conversa; a policy não tem bypass de platform admin; o audit não guarda o texto) |
